...
import {
  HcmAdapter,
  HcmApiError,
} from '../../src/infrastructure/hcm/hcm.adapter';
...
describe('HcmAdapter', () => {
  ...
  it('should successfully get balance', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ employeeId: '1', totalDays: 10 }),
    });
  ...
  it('should successfully file request', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ hcmRequestId: 'HCM-1', status: 'PENDING' }),
    });
  ...
  it('should successfully ingest batch', async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: true,
      json: () => Promise.resolve({ accepted: 5 }),
    });

    const res = await adapter.ingestBatch({
      records: [],
  ...
  it('should throw HcmApiError immediately on 400 error (no retry)', async () => {
    ...
    await expect(adapter.getBalance('1', 'L', 'A')).rejects.toThrow(
      HcmApiError,
    );
